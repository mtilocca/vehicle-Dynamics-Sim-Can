...
XCMG XDE320 Plant Simulator on Zephyr / STM32H753ZI}
...
This document describes the embedded real-time implementation of the XCMG XDE320
...
Physics step (\texttt{s\_plant.step}) & \SI{200--400}{\micro s} & Dugoff + 3-DOF + battery \\
CAN TX (15 frames) & \SI{150--250}{\micro s} & \texttt{can\_send} non-blocking \\
...
\subsection{XCMG XDE320 Parameters}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Vehicle Info} — hardcoded XCMG XDE320 specification table
\end{enumerate}
...
\texttt{vehicle info} & XCMG XDE320 parameters + live surface~$\mu$ \\
...
\subsection{Expected Footprint (Phase 4 complete)}

\begin{center}
\begin{tabular}{lrrl}
\toprule
Region & Budget & Expected & Contents \\
\midrule
Flash (.text + .rodata) & \SI{2}{MB} & $\approx$\SI{450}{KB} & Zephyr kernel, plant code, CAN codec, CSS \\
SRAM (.bss + .data + stacks) & \SI{1}{MB} & $\approx$\SI{380}{KB} & Stacks, heap, net buffers, LOG buffer \\
Heap (STL + Zephyr + net) & \SI{65}{KB} (configured) & $\approx$\SI{40}{KB} & STL containers, net PKT/BUF pool \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Thread Stack Allocation}
...
Thread & Stack \\
\midrule
\texttt{plant\_tid} & \SI{16384}{B} (16~KB) \\
\texttt{http\_tid} & \SI{8192}{B} (8~KB) \\
\texttt{shell\_uart} & \SI{4096}{B} \\
\texttt{main} & \SI{4096}{B} \\
\texttt{stm\_eth} & \SI{1536}{B} \\
\texttt{rx\_q[0]} & \SI{1536}{B} \\
\texttt{can\_rx\_tid} & \SI{1024}{B} \\
\texttt{tcp\_work} & \SI{1024}{B} \\
\texttt{logging} & \SI{1024}{B} \\
\texttt{led\_tid} & \SI{512}{B} \\
\texttt{idle} & \SI{320}{B} \\
\midrule
\textbf{Total stacks} & \textbf{\SI{40768}{B} ($\approx$40~KB)} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{lstlisting}[language=bash, caption={Flash and RAM reports}]
...
[INF] xcmg_sim: XCMG XDE320 plant thread started (dt=10 ms, prio=5)
    \end{verbatim}
