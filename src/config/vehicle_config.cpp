...
VehicleConfig VehicleConfig::load(const std::string& yaml_path) {
    ...
    try {
        ...
        // Parse geometry
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["geometry"]) {
        ...
        // Parse drivetrain
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["drivetrain"]) {
            ...
            vehicle.params.motor_params.max_power_kW = vehicle.params.drive.motor_power_max_w / 1000.0;
            vehicle.params.motor_params.max_torque_nm = vehicle.params.drive.motor_torque_max_nm;
            vehicle.params.motor_params.efficiency = vehicle.params.drive.drivetrain_eff;
        }

        // ====================================================================
        // Parse brakes
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["brakes"]) {
            auto brakes = config["vehicle"]["brakes"];
            vehicle.params.drive.brake_torque_max_nm = brakes["brake_torque_max_nm"].as<double>(4000.0);

            // Store regen efficiencies
        ...
        // Parse battery
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["battery"]) {
        ...
        // Parse resistance
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["resistance"]) {
        ...
        // Parse limits
        // ====================================================================
        if (config["vehicle"] && config["vehicle"]["limits"]) {
        ...
        // ====================================================================
        // Parse wheel rotational dynamics (NEW)
        // ====================================================================
        if (config["wheels"]) {
            auto wheels = config["wheels"];
            vehicle.wheel_params.inertia_kgm2 = wheels["inertia_kgm2"].as<double>(1.0);
        }
        ...
        if (config["tires"]) {
            ...
            if (tires["surface"]) {
                ...
                // Store in PlantModelParams dynamic config (WheelSubsystem will consume this)
                vehicle.params.dynamic_config.surface_mu = vehicle.tire_params.surface.mu_peak;
            }
        ...
        throw std::runtime_error(std::string("[VehicleConfig] YAML parse error: ") + e.what());
...
VehicleConfig VehicleConfig::get_default() {
    ...
    // Wheel defaults (NEW)
    vehicle.wheel_params.inertia_kgm2 = 1.0;
...
void VehicleConfig::print_summary() const {
    ...
    LOG_INFO("Wheel inertia: %.1f kg·m²", wheel_params.inertia_kgm2);
    LOG_INFO("----------------------------------------");
