...
namespace sim
{
    ...
    int SimApp::run_plant_only()
    {
        ...
        for (int k = 0; k < steps; ++k)
        {
            ...
            // --- Open-loop longitudinal scenario (V1)
            // Phase logic is local to sim_app (will move to JSON/Lua later)

            static int phase = 0; // 0=accel, 1=coast, 2=brake
            static double phase_t0 = 0.0;

            if (phase == 0)
            {
                // Accelerate to ~60 km/h
                cmd.drive_torque_cmd_nm = cfg_.motor_torque_nm; // e.g. 1200 Nm
            ...
            else
            {
                // Brake to stop
                cmd.drive_torque_cmd_nm = 0.0;
                cmd.brake_cmd_pct = 40.0; // try 30–50%

                if (s.v_mps < 0.2)
                {
                    cmd.brake_cmd_pct = 0.0; // avoid chatter at standstill
                }
            }
