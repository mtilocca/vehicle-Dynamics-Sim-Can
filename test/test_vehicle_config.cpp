...
// Tests for hardcoded XCMG XDE320 vehicle configuration.
...
TEST(VehicleConfig, DefaultIsXCMG) {
    auto cfg = config::VehicleConfig::get_default();
    EXPECT_EQ(cfg.name, "XCMG XDE320 Electric");
    EXPECT_EQ(cfg.manufacturer, "XCMG");
}

TEST(VehicleConfig, XCMGMassIsCorrect) {
    auto cfg = config::VehicleConfig::get_default();
    EXPECT_NEAR(cfg.params.drive.mass_kg, 218000.0, 1.0);
}

TEST(VehicleConfig, XCMGGeometryIsCorrect) {
    auto cfg = config::VehicleConfig::get_default();
...
TEST(VehicleConfig, XCMGDrivetrainIsCorrect) {
    auto cfg = config::VehicleConfig::get_default();
...
TEST(VehicleConfig, XCMGBatteryIsCorrect) {
    auto cfg = config::VehicleConfig::get_default();
...
TEST(VehicleConfig, XCMGTireIsCorrect) {
    auto cfg = config::VehicleConfig::get_default();
...
TEST(VehicleConfig, LoadFallsBackToDefault) {
    // YAML loading is disabled — load() always returns XCMG defaults
