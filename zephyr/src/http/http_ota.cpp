...
void handle_ota_page(int fd)
{
    ...
    static const char body[] =
        ...
        "function upload(){"
          ...
          "x.onload=function(){"
            ...
            "if(x.status===200){"
              ...
              "set('Upload OK \u2014 '+d.bytes+' bytes written. Click Reboot to apply.','ok');"
        ...
        "}"
        "function reboot(){"
          "set('Rebooting\u2026 reconnect in ~5 s','warn');"
          "document.getElementById('rb').disabled=true;"
          "fetch('/api/reboot',{method:'POST'})"
          ".catch(function(){set('Rebooting (connection lost \u2014 expected)','ok');});"
        "}"
